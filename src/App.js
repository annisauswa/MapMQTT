...
function App() {
    ...
    const [vehicleData, setVehicleData] = useState({ // State for parsed vehicle data
      speed: 'N/A',
      acceleration: 'N/A',
      heading: 'N/A',
      latitude: 'N/A',
    ...
    const handleMqttConnect = ({ broker, port, topic }) => {
      let url = `ws://${broker}:${port}`;
      if (broker === 'broker.hivemq.com' && port === '8000') {
        url = `ws://${broker}:${port}/mqtt`;
      }
      ...
      client.on('message', (topic, message) => {
        ...
        if (dataArray.length === 5) {
          ...
          if (!isNaN(lat) && !isNaN(lng)) {
            ...
            console.warn('Invalid latitude or longitude received:', dataArray[3], dataArray[4]);
          }
        } else {
    ...
    return (
        <div className="App">
            <div className={`mqtt-settings-panel ${showMqttSettings ? '' : 'hidden'}`}>
            ...
            {/* <LocationInput onSetLocation={handleSetLocation} /> */} {/* Removed LocationInput */}
            ...
            <button className="map-style-button" onClick={() => setMapStyle(mapStyle === 'simple' ? 'satellite' : 'simple')}>
              Change Map Style
            </button>
        </div>
    );
